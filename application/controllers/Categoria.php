...
class Categoria extends CI_Controller{
	...
	function start(){
		$result = $this->ini_model->startCategory();
	...
	function register(){
		...
		$insert = $this->ini_model->registerCategory($data);
	...
	function Edition($type){
		if($type == 'bringData'){
			$id = $_POST['id'];
			$sql = $this->ini_model->loadDataCategory($id);
			...
			$update = $this->ini_model->updateCategory($id, $data);
	...
	function delete($id){
        $response = $this->ini_model->deleteCategory($id);
        echo $response;
    }
}