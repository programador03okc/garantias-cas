...
class Tipo extends CI_Controller{
	...
	function start(){
		$result = $this->ini_model->startType();
	...
	function register(){
		...
		$insert = $this->ini_model->registerTypes($data);
	...
	function Edition($type){
		if($type == 'bringData'){
			$id = $_POST['id'];
			$sql = $this->ini_model->loadDataTypes($id);
			
			foreach ($sql as $row) {
				$ids = $row->id_tipo_equipo;
				$nom = $row->descripcion;
			}
			...
			$update = $this->ini_model->updateTypes($id, $data);
	...
	function delete($id){
        $response = $this->ini_model->deleteTypes($id);
        echo $response;
    }
}